...
void screen1()
{
    ...
    drawstring("BNM INSTITUTE OF TECHNOLOGY", -0.35, 0.85, 0.0);

    glColor3f(0.9, 0.9, 0.9);
    drawstring("DEPARTMENT OF INFORMATION SCIENCE AND ENGINEERING", -0.6, 0.75, 0.0);

    glColor3f(0.8, 0.8, 0.8);
    drawstring("-*- VERTICAL LIFT BRIDGE SIMULATION -*-", -0.45, 0.60, 0.0);

    // ---------------- TEAM DETAILS ----------------
    glColor3f(0.2, 0.8, 0.5);
    drawstring("BY:", -0.05, 0.45, 0.0);

    glColor3f(1.0, 1.0, 0.0);
    drawstring("RADHIKA PA (1BG23IS045)", -0.25, 0.35, 0.0);
    drawstring("RAKSHITHA J (1BG23IS046)", -0.25, 0.25, 0.0);

    // ---------------- INSTRUCTIONS ----------------
    glColor3f(0.2, 0.8, 0.5);
    drawstring("INSTRUCTIONS", -0.15, -0.10, 0.0);
    drawstring("----------------", -0.18, -0.15, 0.0);

    glColor3f(0.9, 0.9, 0.9);
    drawstring("* PRESS ENTER TO START  |  ESC TO EXIT *", -0.5, -0.30, 0.0);
    drawstring("PRESS 'S' TO START ANIMATION  |  'T' TO STOP ANIMATION", -0.7, -0.45, 0.0);
    drawstring("PRESS 'D' FOR DAY VIEW  |  'N' FOR NIGHT VIEW", -0.6, -0.60, 0.0);
...
void screen3()
{
    // Set background to a warm orange tone
  glClearColor(0.0, 0.5, 0.4, 0.0);  // teal-green background
    ...
    // Main message
    drawstring("* * *  THANK YOU  * * *", -0.25, 0.1, 0.0);

    // Separator line
    drawstring("-------------------------", -0.30, -0.05, 0.0);
...
void lighthouse()
{
    ...
    // ===== Bulb on top =====
    glPushMatrix();
    glTranslatef(0.275, 0.05, -5.0);

    // Day → Dim light
    if (flagd == 1)
        glColor3f(1.0, 1.0, 0.0);  
    else
        glColor3f(1.0, 0.9, 0.4);  // warmer color at night

    glutSolidSphere(0.03, 30, 30);

    // Add a glowing aura only at night
    if (flagd == 0)
    {
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        glColor4f(1.0, 1.0, 0.6, 0.3); // faint glow layer
        glutSolidSphere(0.06, 30, 30);
        glColor4f(1.0, 1.0, 0.3, 0.2); // outer glow
    ...
    // ===== Rotating Light Beam (only at night) =====
    if (flagd == 0)
    {
        glPushMatrix();
        glTranslatef(0.275, 0.05, -5.0);  // same bulb position
        glRotatef(lighthouseAngle, 0, 0, 1);

        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        glBegin(GL_TRIANGLES);
        glColor4f(1.0f, 0.9f, 0.2f, 0.6f); // warm bright yellow
 // soft yellow light beam
...
void update(int value)
{
    if(flagx==1)
    {
     ...
     if(flagp==1)
     {
      ...
      if (flagd == 0) { // only rotate at night
    lighthouseAngle += 2.0f;
    if (lighthouseAngle > 360.0f)
...
void display()
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    if(flagd == 1)
    glClearColor(0.0, 0.5, 1.0, 0.0);   // Day sky (blue)
else
    glClearColor(0.0, 0.0, 0.1, 0.0);   // Night sky (dark blue)
...
{
    glColor3f(0.9, 0.9, 1.0);   // Moon (white)
    glTranslatef(-1.0, 0.8, -5.1);
}
...
void mykeyboard(unsigned char key,int x,int y)
{
    switch(key)
    {
        ...
        case 115:  // 's'
            if(flag == 1)
                flagx = 1;  // Start animation
            break;

        case 84:   // 'T'
        case 116:  // 't'
            flagx = 0;      // Stop animation
            break;
